...
class LaserScan:
  ...
  def set_roi(self):
    ...
    #mask = np.ones(len(PointCloud),dtype=np.uint8)
    assert isinstance(self.roi,list)
    mask_list = []
    for roi in self.roi:
      region_mask_list = []
      ...
      if 'xmin' in roi:
        mask = mask * PointCloud[:, 0] >= roi["xmin"]

      if 'xmax' in roi:
        mask = mask * PointCloud[:, 0] <= roi["xmax"]

      if 'ymin' in roi:
        mask = mask * PointCloud[:, 1] >= roi["ymin"]
      
      if 'ymax' in roi:
        mask = mask * (PointCloud[:, 1] <= roi["ymax"])
