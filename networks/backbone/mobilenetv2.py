...
def match_models_param(target_model,weights):
...
class MobileNetV2(nn.Module):
    def __init__(self, in_channels = 32, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        """
...
def mobilenet_v2(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['mobilenet_v2'],
                                              progress=progress)
        state_dict = match_models_param(model,state_dict)
